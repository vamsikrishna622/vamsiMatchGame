...
class MatchGame extends Component {
  ...
  onResetTimer = () => {
    clearInterval(this.intervalId)
    this.runTime()
    this.setState({...initialState, randomImg: imagesList[0].imageUrl})
  }
  ...
  incrementTimeElapsedInSeconds = () => {
    ...
    if (isTimerCompleted) {
      clearInterval(this.intervalId)
      this.setState({isTimerRunning: false, isGameEnded: true})
    } else {
      this.setState(prevState => ({
  ...
  render() {
    ...
    const filteredImagesList = this.filterActiveCategoryImages()

    return (
      <div className="app-container">
        <Header score={score} seconds={timeElapsedInSeconds} />

        <div className="responsive-container">
          <div className="random-image-container">
            {isGameEnded ? (
              <ScoreCard score={score} onResetTimer={this.onResetTimer} />
            ) : (
              <>
                ...
                <ul className="image-list-container">
                  {filteredImagesList.map(imageDetails => (
                    <ImageList
                      ...
                      checkingThumbnail={this.checkingThumbnail}
                    />
                  ))}
                </ul>
              </>
